...
def book_search(request, query):
    ...
    print ("books:" , books)
    return render(request, '../templates/library/search.html',{'search_type':'book', 'books': books })

@login_required(login_url='/signin/')
def author_search(request, query):
    authors = Authors.objects.filter(name__icontains=query)
    print ("authors:", authors)
    return render(request,'../templates/library/search.html', {'search_type':'author', 'authors':authors})
